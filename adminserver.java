...
public class adminserver {
	...
	{
		ServerSocket adminRequest=new ServerSocket(11123);		
		boolean flag=true;
		while(flag)
		{
			
			System.out.println("waiting for admin request");
...
//				        System.out.println("login time is  "+hour+" : "+minute+" : "+second);
				       ...
				       // st[11]="success";
						...
						if(db.adduser(st[1],st[2],st[3],st[4],st[5],st[6],st[7]))
						{
							ps.println("SUCCESS");
					...
					case "UPDATE" :
						...
						if(db.updateusers(st[1], st[2], st[3], st[4], st[5], st[6], st[7], st[8], st[9], st[10], st[11]))
						{
							ps.println("SUCCESS");
